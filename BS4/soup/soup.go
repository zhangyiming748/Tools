...
func GetWithClient(url string, client *http.Client) (string, error) {
	...
	log.Info.Printf("请求时使用的请求头为: %v", req.Header.Get("User-Agent"))
...
// Post With Proxy
func PostWithProxy(url string, bodyType string, body interface{}, proxy string) (string, error) {
	u, err := netURL.Parse(proxy)
	if err != nil {
		log.Warn.Fatal("parse url error: ", err)
	}
	log.Info.Printf("使用魔法..biu..主机: %v\n", u.Host)
	client := http.Client{
...
func GetWithProxy(url, proxy string) (string, error) {
	...
	// url, err := url.Parse("http://username:password@inproxy.sjtu.edu.cn:8000")
